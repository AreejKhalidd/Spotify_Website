...
/**
 * Calendar class to allow artist to schedule his events to send to users
 */
...
export default class Cal extends React.Component {
    ...
    /**
     * componentDidMount here is used to get the events the user has
     * componentWillUnmount here send all the events after addition and deletion to minimize traffic "sending one request when removing the component from DOM"
     * and not after every addition or deletion   
     */
    
    // Mocking
    // componentDidMount(){
    //     fetch("http://127.0.0.1:8080/calendarevents.json")
    //     .then((response) => response.json())
    //     .then((calendar) => this.setState({ events: calendar }));
    // }
    // componentWillUnmount(){
    //     // axios.post('http://127.0.0.1:8080/calendarevents.json', this.state.events)  
    // }
    ...
    render () {
        return (
            <div>
                <mobiscroll.Form ref={this.setForm}>
                        ...
                        <mobiscroll.FormGroup>  
                            <mobiscroll.Switch value={this.state.allDay} onChange={this.allDayChange}>
                                ...
                                <mobiscroll.Range
                                    ...
                                    showSelector={false}
                                />
                            <mobiscroll.Segmented name="group" value="busy" checked={this.state.isFree === 'busy'} onChange={this.isFreeChange}>
                                Show as busy
                            </mobiscroll.Segmented>
                            <mobiscroll.Segmented name="group" value="free" checked={this.state.isFree === 'free'} onChange={this.isFreeChange}>
                                Show as free
                            </mobiscroll.Segmented>
                        </mobiscroll.FormGroup>
                    </mobiscroll.Form>
